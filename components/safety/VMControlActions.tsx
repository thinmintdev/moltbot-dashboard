...
interface VMControlActionsProps {
  vmId: string;
  vmName: string;
  vmStatus: 'online' | 'offline' | 'unknown';
...
// Map VM actions to the operation types defined in the safety system
const vmActionToOperationType: Record<string, OperationType> = {
  start: 'query', // Start is safe, treated as query
  stop: 'stop',
  restart: 'restart',
  reboot: 'reboot',
  delete: 'delete',
...
function ActionButton({
  ...
  const target = { type: 'vm' as const, id: targetId, name: targetName };
  ...
  return (
    <>
      ...
      <ConfirmationDialog
        ...
        title={`Confirm ${label}`}
        message={`Are you sure you want to ${label.toLowerCase()} "${targetName}"? This operation has a ${riskLevel} risk level.`}
        riskLevel={riskLevel}
...
export function VMControlActions({
  vmId,
  vmName,
  vmStatus,
  onStart,
  ...
  showLabels = true,
  compact = false,
}: VMControlActionsProps) {
  const isOnline = vmStatus === 'online';
  const isOffline = vmStatus === 'offline';

  // Default handlers that log a warning
  const createDefaultHandler = (action: string) => async () => {
    console.warn(`No handler provided for ${action} on VM ${vmId}`);
  };

  return (
    <div className={classNames('flex items-center gap-2', className)}>
      {/* Start Button - only show when offline */}
      {isOffline && onStart && (
        <ActionButton
          label="Start"
      ...
      {/* Restart Button - only show when online */}
      {isOnline && onRestart && (
        <ActionButton
          label="Restart"
      ...
      {/* Stop Button - only show when online */}
      {isOnline && onStop && (
        <ActionButton
          label="Stop"
      ...
      {/* Reboot Button - only show when online */}
      {isOnline && onReboot && (
        <ActionButton
          label="Reboot"
          icon={<Power size={compact ? 14 : 16} />}
          operationType="reboot"
          targetId={vmId}
          targetName={vmName}
          onExecute={onReboot || createDefaultHandler('reboot')}
          variant="danger"
          showLabel={showLabels}
          compact={compact}
        />
      ...
      {/* Delete Button - always available but dangerous */}
      {onDelete && (
        <ActionButton
          label="Delete"
          ...
          targetId={vmId}
          targetName={vmName}
          onExecute={onDelete || createDefaultHandler('delete')}
          variant="danger"
          requiresCheckbox
          showLabel={showLabels}
          compact={compact}
...
export default VMControlActions;
